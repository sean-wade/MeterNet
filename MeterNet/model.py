...
class FaceModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        #x = self.model.fc(x)    #comment by zhanghao
        x = self.model.fc1(x)
        x = self.model.fc2(x)
...
class FaceModelCenter(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.model.fc2(x)
        self.features = x                       #comment by zhanghao
        #self.features_norm = self.l2_norm(x)   #comment by zhanghao
        #return self.features_norm              #comment by zhanghao
        return self.features
